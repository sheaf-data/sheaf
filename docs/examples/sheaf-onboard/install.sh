#!/usr/bin/env bash
# Install the sheaf-onboard skill for Claude Code.
#
# Copies SKILL.md + PROCEDURE.md + README.md into
# ~/.claude/skills/sheaf-onboard/ so the skill is available in every Claude
# Code conversation, regardless of which directory the session is rooted in.
#
# Idempotent — re-run after editing any source file to refresh the install.
#
# Optional flags:
#   --dry-run       show actions, don't execute
#   --dest PATH     override install dir (default: ~/.claude/skills/sheaf-onboard)
#   --uninstall     remove the installed skill instead of installing it

set -euo pipefail

SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEST_DIR="${HOME}/.claude/skills/sheaf-onboard"
DRY_RUN=0
UNINSTALL=0
...
installed sheaf-onboard → $DEST_DIR
...
It requires a built 'sheaf' binary with the 'verify' subcommand on PATH
(go build -o sheaf ./cmd/sheaf). Re-run this script after editing the
source files to refresh the install.
EOF
